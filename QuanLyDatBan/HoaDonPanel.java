...
public class HoaDonPanel extends javax.swing.JPanel {
    ...
    public HoaDonPanel() {
        ...
        placeholderSupport.addPlaceholder(txt_maban,"Nhập mã bàn");
        placeholderSupport.addPlaceholder(txt_mahd,"Nhập mã hóa đơn");
        placeholderSupport.addPlaceholder(txt_sdt,"Nhập số điện thoại");
...
//        JTable_ChiTiet.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
    ...
    private void initComponents() {
        ...
        txt_maban.setFont(new java.awt.Font("Segoe UI", 0, 16)); // NOI18N
        txt_maban.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                txt_mabanActionPerformed(evt);
            }
        });
        ...
        jComboBox_TrangThai.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Mặc Định", "Đã Thanh Toán", "Chưa Thanh Toán" }));
                        ...
                        .addGroup(jp_TTLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            .addGroup(jp_TTLayout.createSequentialGroup()
                                ...
                                .addGroup(jp_TTLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING, false)
                                    .addComponent(txt_sdt, javax.swing.GroupLayout.Alignment.LEADING, javax.swing.GroupLayout.DEFAULT_SIZE, 167, Short.MAX_VALUE)
                                    ...
                                    .addComponent(txt_maban, javax.swing.GroupLayout.Alignment.LEADING)
        ...
        jTable_ChiTiet.getTableHeader().setReorderingAllowed(false);
        ...
        JTable_DS.getTableHeader().setReorderingAllowed(false);
        ...
        if (JTable_DS.getColumnModel().getColumnCount() > 0) {
            JTable_DS.getColumnModel().getColumn(0).setResizable(false);
            JTable_DS.getColumnModel().getColumn(1).setResizable(false);
            JTable_DS.getColumnModel().getColumn(2).setResizable(false);
            JTable_DS.getColumnModel().getColumn(3).setResizable(false);
            JTable_DS.getColumnModel().getColumn(4).setResizable(false);
        }
    ...
    private void txt_mabanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_txt_mabanActionPerformed
        // TODO add your handling code here:
        
    }//GEN-LAST:event_txt_mabanActionPerformed
    ...
    private void jButtonTimKiemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonTimKiemActionPerformed
        ...
        boolean kt_maban = true;
        boolean kt_sdt = true;
        boolean kt_mahd = true;
        if(maban.equals("Nhập mã bàn")){
            kt_maban=false;
        }
        if(sdt.equals("Nhập số điện thoại")){
            kt_sdt=false;
        }
        if(mahd.equals("Nhập mã hóa đơn")){
            kt_mahd=false;
        }
        DefaultTableModel tableModel = (DefaultTableModel) JTable_DS.getModel();
        tableModel.setRowCount(0);
        hoadon_dao = new HoaDonDAO();
        List<HoaDonThanhToan> dshd = hoadon_dao.getDanhSachHoaDon();
        if(!kt_maban && !kt_sdt && !kt_mahd){
            JOptionPane.showMessageDialog(
            null, // Để null nếu không có thành phần cha
            "Không để rổng tất cả thông tin rỗng", // Thông điệp hiển thị
        ...
        for (HoaDonThanhToan hd : dshd) {
                ...
                if(kt_sdt){
                    if(!hd.getKhachHang().getSoDienThoai().equals(sdt)){
                        continue;
                    }
                }
                if(kt_maban){
                    if(!hd.getBanAn().getMaBan().equals(maban)){
                        continue;
                    }
                }
                Object[] rowData = {
    ...
    private void btn_InActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_InActionPerformed
        ...
        if(row==-1){
            JOptionPane.showMessageDialog(null, "Chưa chọn hóa đơn!", "Thông báo", JOptionPane.INFORMATION_MESSAGE);
            return;
        }
...
// Chọn nơi lưu file PDF
    ...
    if (userSelection == JFileChooser.APPROVE_OPTION) {
        ...
        try {
            ...
            DefaultTableModel tableModel = (DefaultTableModel) JTable_DS.getModel();
            hoadon_dao = new HoaDonDAO();
            String mahd= String.valueOf(tableModel.getValueAt(row, 0));
            HoaDonThanhToan hd = hoadon_dao.timHoaDonTheoMa(mahd);
            // Thêm tiêu đề
            document.add(new Paragraph("Nhà hàng WAVE", FontFactory.getFont(FontFactory.HELVETICA_BOLD, 16)));
            document.add(new Paragraph("12 Nguyễn Văn Bảo, phường 4, quận Gò Vấp, TP. Hồ Chí Minh", FontFactory.getFont(FontFactory.HELVETICA, 12)));
            document.add(new Paragraph("---------------------------------------------------------------------------------------------------"));
            document.add(new Paragraph("HÓA ĐƠN BÁN HÀNG", FontFactory.getFont(FontFactory.HELVETICA_BOLD, 14)));
            document.add(new Paragraph("Mã hóa đơn: " + hd.getMaHoaDon(), FontFactory.getFont(FontFactory.HELVETICA, 12)));
            document.add(new Paragraph("Ngày đặt bàn: " + hd.getNgayDatBan(), FontFactory.getFont(FontFactory.HELVETICA, 12)));
            document.add(new Paragraph("Bàn: " + hd.getBanAn().getMaBan(), FontFactory.getFont(FontFactory.HELVETICA, 12)));
            document.add(new Paragraph("Khách hàng: " + hd.getKhachHang().getTenKhachHang(), FontFactory.getFont(FontFactory.HELVETICA, 12)));
            document.add(new Paragraph("Thu ngân: " + hd.getNhanVien().getTenNhanVien(), FontFactory.getFont(FontFactory.HELVETICA, 12)));
             document.add(new Paragraph("Mã hóa đơn: " + "123", FontFactory.getFont(FontFactory.HELVETICA, 12)));
document.add(new Paragraph("\n"));

            // Thêm bảng chi tiết món ăn
            ...
            ArrayList<ChiTietHoaDon> ListChiTietHoaDon = (ArrayList<ChiTietHoaDon>) chitiet_dao.getDanhSachChiTietHoaDon(mahd);
            DecimalFormat df = new DecimalFormat("#,###,### VNĐ");
            for (ChiTietHoaDon chiTiet : ListChiTietHoaDon) {
                table.addCell(chiTiet.getMonAn().getTenMonAn());
                table.addCell(String.valueOf(chiTiet.getSoLuong()));
                table.addCell(df.format(chiTiet.getMonAn().getGiaTien()));
                table.addCell(df.format(chiTiet.getSoLuong()*chiTiet.getMonAn().getGiaTien()));
            }

            document.add(table);

            // Thêm phần tổng kết
            document.add(new Paragraph("Tổng tiền hàng: " + df.format(hd.getTongTien())));
            document.add(new Paragraph("Tiền giảm giá: " + hd.getKhuyenMai().getGiamGia()));
            document.add(new Paragraph("Thuế VAT: " + hd.getVAT()));
            document.add(new Paragraph("Tổng cộng: " + df.format(hd.getTongTien())));
            document.add(new Paragraph("\nCảm ơn quý khách và hẹn gặp lại!", FontFactory.getFont(FontFactory.HELVETICA_BOLD, 12)));

            // Đóng tài liệu
            document.close();

            // Hiển thị thông báo
            JOptionPane.showMessageDialog(this, "Hóa đơn đã được in ra file PDF thành công!\nLưu tại: " + filePath);
            ...
            if (Desktop.isDesktopSupported()) {
                ...
                JOptionPane.showMessageDialog(this, "Không thể mở file PDF tự động. Vui lòng mở file tại: " + filePath);
            }
        } catch (Exception e) {
            e.printStackTrace();
            JOptionPane.showMessageDialog(this, "Đã xảy ra lỗi khi in hóa đơn ra PDF: " + e.getMessage());
        }
    }
...
//    String mahd = txt_mahd.getText();
    ...
    hoadon_dao = new HoaDonDAO();
    List<HoaDonThanhToan> dsHoaDon = hoadon_dao.getDanhSachHoaDon(); // Gọi hàm để lấy danh sách hóa đơn
    // Xóa dữ liệu cũ trong bảng
    DefaultTableModel tableModel = (DefaultTableModel) JTable_DS.getModel();
    tableModel.setRowCount(0); // Xóa dữ liệu cũ

    // Duyệt qua danh sách hóa đơn và thêm vào bảng nếu trạng thái khớp
    if(trangthai.equals("Mặc Định")){
    ...
    if(trangthai.equals("Đã Thanh Toán")){
        for (HoaDonThanhToan hd : dsHoaDon) {
            if(hd.getTrangThai()==EnumTrangThaiDatBan.DA_THANH_TOAN){
                Object[] rowData = {
                    hd.getMaHoaDon(),
                    hd.getNgayDatBan(),
                    hd.getBanAn().getMaBan(),
    ...
    if(trangthai.equals("Chưa Thanh Toán")){
        for (HoaDonThanhToan hd : dsHoaDon) {
            if(hd.getTrangThai()==EnumTrangThaiDatBan.DA_DAT){
                Object[] rowData = {
                    hd.getMaHoaDon(),
                    hd.getNgayDatBan(),
                    hd.getBanAn().getMaBan(),
    ...
    private void btn_ThanhToanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_ThanhToanActionPerformed
        ...
        if(String.valueOf(tableModel.getValueAt(row, 4))=="DA_THANH_TOAN"){
            JOptionPane.showMessageDialog(null, "Hóa đơn đã thanh toán!", "Thông báo", JOptionPane.INFORMATION_MESSAGE);
            return;
        }
        hoadon_dao.capNhatTrangThaiHoaDon(String.valueOf(tableModel.getValueAt(row, 2)), String.valueOf(tableModel.getValueAt(row, 0)));
        JOptionPane.showMessageDialog(null, "Thanh toán thành công!", "Thông báo", JOptionPane.INFORMATION_MESSAGE);
        tableModel.setRowCount(0);
        List<HoaDonThanhToan> dsHoaDon = hoadon_dao.getDanhSachHoaDon();
        for (HoaDonThanhToan hd : dsHoaDon) {
                Object[] rowData = {
                    hd.getMaHoaDon(),
                    hd.getNgayDatBan(),
